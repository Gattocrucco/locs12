...
table = np.load('temps1series0214.npy')
...
for wname in ['efficiency', 'fakerate', 'effvsrate', 'effvssigma']:
    ...
    fig, ax = plt.subplots(2, 2, num=f'temps1series0214plot-{wname}', **figkw)
    figs.append(fig)
    axs.append(ax)
axs = np.array(axs)
...
for ivl in range(table.shape[1]):
    ...
    for ifilter, fname in enumerate(['slow', 'fast']):
        qax = axs[:, ifilter, ivl]
        ...
        for ifig, ax in enumerate(qax):
            ...
            for isigma, entry in enumerate(entries[inphotons]):
                ...
                if len(changepoint) == 0:
                    # in this case all y are equal
                    ax.axhline(y[0], **plotkw)
                else:
        ...
        for ax in qax:
            s1type = 'slow' if entries[0, 0]['parameters']['VL'] < 1 else 'fast'
