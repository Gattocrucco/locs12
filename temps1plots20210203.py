...
if __name__ == '__main__':
    ...
    generator = np.random.default_rng(202102030836)
    ...
    for argstruct in tqdm.tqdm(arglist):
        ...
        sim = Simulation(**kw)
        figs = [
            sim.plot_filter_performance_threshold(),
            sim.plot_filter_performance(),
            sim.plot_filter_output_histogram(),
            sim.plot_temporal_distribution(),
        ]
        
        for fig in figs:
            figname = fig.canvas.get_window_title()
            filename = figname.replace('temps1.Simulation.', '').replace('plot_', '').replace('filter_', '')
